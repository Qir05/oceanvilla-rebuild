...
import type { FeaturedVillaCardData } from "./page";
...
function ListingCard({ l }: { l: FeaturedVillaCardData }) {
  const title = l.displayName;
  const subtitle = l.subtitle;
  const hero = l.heroImageUrl;
...
export default function HomeClient({
  ...
  initialListings: FeaturedVillaCardData[];
  occupancyAnswer: string;
}) {
  ...
  // Server-fetched (Tier 1, revalidated every 5 minutes) — see app/page.tsx.
  // Already reduced server-side to FeaturedVillaCardData; this section has
  // never displayed live price/availability, so no client-side refetch of
  // this data is needed.
      ...
      <section id="featured" className="py-16 md:py-20 bg-white">
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
          ...
          <div className="mt-10 md:mt-12">
            {listings.length === 0 ? (
              // A Hostaway fetch failure (unreachable, erroring, or every
              // listing failed) is not "no villas exist" — that would be a
              // data-modeling bug, not a real state. This renders as a
              // normal 200 response with its own recovery UI instead of a
              // permanent loading spinner or a raw error string, matching
              // the same pattern used on /rentals and /listing/[id].
              <div className="text-center py-20 px-4 md:px-0">
                <div className="mx-auto max-w-xl rounded-2xl border border-red-200 bg-red-50 p-8">
                  <h3 className="text-lg font-semibold text-red-800">Villa information is temporarily unavailable</h3>
                  <p className="mt-3 text-sm text-red-700">
                    We&apos;re having trouble loading our featured villas right now. This is a temporary issue on our
                    end — please try again in a moment, or browse the full collection directly.
                  </p>
