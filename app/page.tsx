...
export default function Home() {
  ...
  async function onSearch() {
    ...
    try {
      // ✅ Later: wire this to Hostaway quote endpoint:
      // GET /api/hostaway/quote?listingId=...&checkIn=...&checkOut=...&guests=...
      // const res = await fetch(`/api/hostaway/quote?...`, { cache: "no-store" });
      ...
      <header className="sticky top-0 z-50 border-b border-white/10 bg-[#071421]/70 backdrop-blur-[14px]">
        <div className="mx-auto flex h-16 max-w-6xl items-center gap-4 px-4">
          ...
          <div className="hidden items-center gap-3 md:flex">
            <a className="text-sm text-white/70 hover:text-white transition" href={`tel:${BRAND.phone}`}>
              {BRAND.phone}
            </a>
        ...
        {mobileMenuOpen ? (
          <div className="md:hidden border-t border-white/10 bg-[#071421]/85 backdrop-blur-[14px]">
            <div className="mx-auto max-w-6xl px-4 py-4 grid gap-2 text-sm text-white/80">
              <Link onClick={() => setMobileMenuOpen(false)} className="rounded-2xl px-3 py-2 hover:bg-white/5" href="/rentals">
                Rentals
              </Link>
              <Link onClick={() => setMobileMenuOpen(false)} className="rounded-2xl px-3 py-2 hover:bg-white/5" href="/amenities">
                Amenities
              </Link>
              <Link onClick={() => setMobileMenuOpen(false)} className="rounded-2xl px-3 py-2 hover:bg-white/5" href="/location">
                Location
              </Link>
              <Link onClick={() => setMobileMenuOpen(false)} className="rounded-2xl px-3 py-2 hover:bg-white/5" href="/about">
                About
              </Link>
              <Link onClick={() => setMobileMenuOpen(false)} className="rounded-2xl px-3 py-2 hover:bg-white/5" href="/contact">
                Contact
              </Link>
